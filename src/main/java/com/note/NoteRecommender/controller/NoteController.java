...
public class NoteController {
    ...
    @DeleteMapping("/user/{uId}/dep/{depId}/sem/{semId}/cou/{couId}/note/{noteId}/delete")
    public ResponseEntity<?> deleteNote(@Valid @PathVariable("uId")Long uId, @PathVariable("depId")Long depId,@PathVariable("semId")Long semId,@PathVariable("couId")Long couId,@PathVariable("noteId")Long noteId) throws Exception {
        try {
            String message=this.noteService.deleteNote(uId,depId,semId,couId,noteId);
            return  new ResponseEntity<>(message,HttpStatusCode.valueOf(200));
        }catch (Exception e){
            return new ResponseEntity<>(new ApiResponse(e.getMessage(),false),HttpStatusCode.valueOf(500));
        }
    }
    ...
    @GetMapping("/approveNote/{noteId}")
    @PreAuthorize("hasAuthority('approve_note')")
    public ResponseEntity<ApiResponse> approveNote(@PathVariable Long noteId){
        String message=this.noteService.approveNote(noteId);
        return new ResponseEntity<>(new ApiResponse(message,true),HttpStatusCode.valueOf(200));
    }

    @GetMapping("/rejecNote/{noteId}")
    @PreAuthorize("hasAuthority('reject_note')")
    public ResponseEntity<ApiResponse> rejectNote(@PathVariable Long noteId){
        String message=this.noteService.rejectNote(noteId);
        return new ResponseEntity<>(new ApiResponse(message,true),HttpStatusCode.valueOf(200));
    }
